    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s CPTL %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s CPTL %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
// CapitalCoin: get current stake weight
 ...
 * TODO: performance comparison
    ...
    {
        if (!SelectCoinsSimple(nBalance - nReserveBalance, GetAdjustedTime(), nCoinbaseMaturity * 20, setCoins, nValueIn))
            return false;
...
*/
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        if (nTimeWeight == nStakeMaxAge)
        {
			...
			printf("TimeWeight of %"PRI64d"has reached StakeMaxAge of: %d, The Maximum weight of: %"PRI64d" was reached\n", nTimeWeight, nStakeMaxAge, nMaxWeight);
        }
    }
